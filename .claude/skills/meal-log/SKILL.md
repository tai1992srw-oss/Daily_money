...
3. **記録**: 以下のように POST する（`-L` 必須。GASはリダイレクトを返す）:

```bash
curl -sL -X POST '<api_url>' \
  -H 'Content-Type: application/json' \
  -d '{"token":"<token>","action":"addMeal","meal":"昼食","description":"唐揚げ定食とビール1杯","kcal":1050,"protein_g":38,"fat_g":45,"carbs_g":95,"note":"ビール中ジョッキ含む"}'
```
