...
1. **食事内容の解釈**: 引数（または直前のユーザー発言）から食事内容を把握する。
   - 区分（朝食/昼食/夕食/間食）は発言や現在時刻(JST)から推定する（〜10時:朝食、10〜15時:昼食、15〜18時:間食、18時〜:夕食）
   - 「昨日の夜」など過去の食事なら date/time を調整する
...
- ユーザーが「まとめ」「振り返り」を求めたら `?action=summary&days=7` を使って週次の講評をする
