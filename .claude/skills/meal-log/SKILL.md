...
## 目標カロリー（変動制・2026-09-09のウォッチ検証で採用）

その日の目標摂取は **「ウォッチ総消費(total_kcal) − 500kcal」の変動制**。残り枠もこれ基準で案内する。

- 2週間の検証で「ウォッチ収支の累積」と「実測体重変化」の誤差が1日約150kcalと小さかったため、ウォッチを全面採用した
- 消費がまだ伸びていない時間帯（朝〜昼）は「昨日並みに動けば消費◯◯くらい、なら残り◯◯」のように見込みで案内してよい
- 摂取の下限は1,500kcal（それ以下に削らない）。体重は7日平均で判断
- **ウォッチを着け忘れた日は従来の固定1,750kcalに戻す**
- 飲み会日も同ルール（収支マイナスなら成功）。杯数制限は課さない（ユーザーとの合意事項）
...
- **1件に何枚でも付けられる**。写真が複数枚あるときは1枚ずつアップロードし、`photo_url` に
  **カンマ区切り**で並べる（`"photo_url":"<url1>,<url2>"`）。アプリはサムネイルに枚数バッジを出し、
  タップすると横スワイプで切り替わる
...
- すでに写真がある行に**足す**ときは `"photo_append":true` を付ける（既存の写真を消さずに
  後ろに追加する。同じ URL は重複しない）。付けないと差し替えになる
...
ユーザーが味の感想（「美味かった〜」「味薄かった」「ネギが辛い」など）を書いたら、
addMeal の `comment` にそのまま入れる（`note` はカロリー計算の根拠用なので混ぜない）。
「星4」「⭐⭐⭐⭐」のように評価を明示していたら `rating`（1〜5 の整数）も入れる。

食後に感想だけ送られてきたら updateMeal で後付けする:

```json
{"token":"<token>","action":"updateMeal","comment":"スープ濃厚で美味い","rating":4}
```

（その日の最後の食事行が対象。別の行に付けるなら `"time":"12:30"` か `"row":15` を足す）

- 感想は本人の言葉をなるべくそのまま残す（あとでレビュー日記として見返すため、要約しすぎない）
- `rating` は本人が数字や星で明示したときだけ入れる。感想の温度感から勝手に推定しない
- アプリでは食事カードに ★ と 💬 で表示され、お店タブの「うまい順」ソートと
  店ごとの平均評価（★x.x）に使われる
